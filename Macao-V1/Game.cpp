...
Game::Game()
	: m_human(0, "You"), m_ai(1, "AI"), m_cardsToDraw(0), m_skipNext(false),
	m_gameRunning(true) {
}
...
void Game::startGame() { m_discardPile.push(m_deck.drawCard()); }
...
void Game::playerTurn() {
	...
	for (const auto& card : m_human.getHand()) {
		if (card.isValidCard(m_discardPile.top(), m_cardsToDraw)) {
			hasValidCard = true;
			break;
		}
	}
	...
	if (!hasValidCard) {
		...
		if (m_cardsToDraw > 0) {
			std::cout << "You cannot counter the penalty! Auto-drawing " << drawCount
				<< " cards.\n";
		}
		else {
			std::cout << "You have no valid cards! Auto-drawing 1 card.\n";
		}
...
char Game::aiChooseSuit() {
	...
	if (maxCount <= 0 || (bestSuit != 'T' && bestSuit != 'R' && bestSuit != 'N' &&
		bestSuit != 'C')) {
...
std::vector<Card>
Game::ensureValidFirstCard(const std::vector<Card>& selectedCards) {
...
void Game::aiTurn() {
	...
	if (!cardsToPlay.empty()) {
		...
		std::cout << "AI has " << m_ai.getHand().size()
			<< " card(s) left in hand.\n";
	}
	else {
		int drawCount = std::max(1, m_cardsToDraw);
		...
		std::cout << "AI now has " << m_ai.getHand().size()
			<< " card(s) in hand.\n";
